...
async def transcribe_channel_voice(message: types.Message):
    try:
        ...
        await message.answer("Произошла неожиданная ошибка при обработке аудиосообщения в канале. Пожалуйста, попробуйте позже.", reply_to_message_id=message.message_id)